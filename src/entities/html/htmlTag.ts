...
export class HTMLTagSpecification {
  constructor(public name: string, public description: string, public attributes: string[] = []) { }
}

export interface HTMLTagSet {
  [tag: string]: HTMLTagSpecification;
}

interface HTMLValueSets {
  [tag: string]: string[];
}

export const HTML_EMPTY_ELEMENTS: string[] = ["area", "base", "br", "col", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr"];

export const HTML_TAGS: HTMLTagSet = {
  // The root element
...
export const valueSets: HTMLValueSets = {
  b: ["true", "false"],
...
export function getHTMLAttributes(htmlTag: HTMLTagSpecification, includeGlobal = true): string[] {
  let attributes: string[] = htmlTag.attributes;
  if (includeGlobal) {
...
export function getHTMLAttributeNames(htmlTag: HTMLTagSpecification, includeGlobal = true): string[] {
  let attributes: string[] = htmlTag.attributes;
  if (includeGlobal) {
    attributes = attributes.concat(globalAttributes);
  }

  return attributes.map((attr: string) => attr.split(":")[0]);
}

export function constructHTMLAttributeSnippet(htmlTag: HTMLTagSpecification, attributeName: string, attributeQuoteType: AttributeQuoteType = AttributeQuoteType.Double): string {
